...
u32 LCD_ReadPoint(u16 x,u16 y)
{
	...
	opt_delay(2);
    b = LCD_RD_DATA();
    g = r & 0XFF;               //7789, 第一次读取的是RG的值,R在前,G在后,各占8位
    g <<= 8;
    return (((r >> 11) << 11) | ((g >> 10) << 5) | (b >> 11));  //7789需要公式转换一下
}

//LCD开启显示
void LCD_DisplayOn(void)
{					                        
	LCD_WR_REG(0X29);      //7789发送开启显示指令
}

//LCD关闭显示
void LCD_DisplayOff(void)
{	   
	LCD_WR_REG(0X28);       //7789发送关闭显示指令 
}
...
{ 	  
	...
	//初始化完成以后,提速
	if(lcddev.id==0x7789)		//则设置WR时序为最快
    {
        //重新配置写时序控制寄存器的时序
...
//(x,y):中心点
//r    :半径
void LCD_Draw_Circle_defalutColor(u16 x0,u16 y0,u8 r)
{
    int a, b;
    int di;
    a = 0;
